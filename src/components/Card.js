...
const Card = ({ movie, ...props }) => {
  ...
  return (
    <div onClick={() => history.push(`/movie/${movie?.id}`)} className={`group animate-loading-fade w-36 h-64 xs:w-52 xs:h-96 flex flex-col bg-gray-100 hover:bg-gray-200 rounded-md my-5 mx-auto p-3 transition-colors cursor-pointer shadow-md overflow-hidden ${props.similar && `mb-3 mt-0 mr-3`}`}>
      ...
      <div className="h-full mt-3 flex flex-col justify-between">
        <div>
          <h2 className="text-gray-700 group-hover:text-gray-900 transition-colors text-xs xs:text-base">{movie?.title.length > 38 ? (movie.title.substr(0, 38) + '...') : movie.title}</h2>
        </div>
        <div>
